...
async function getFacultades(req: Request, res: Response) {
  try {
...
async function getFacultadById(req: Request, res: Response) {
  const { id } = req.params;
  try {
    const result = await pool.query(
...
async function createFacultad(req: Request, res: Response) {
  ...
  try {
    ...
    if (isDatabaseError(error) && error.code === '23505') {
      return res
        .status(400)
        .json({ message: 'El código de facultad ya existe' });
    }
...
async function updateFacultad(req: Request, res: Response) {
  const { id } = req.params;
...
async function deleteFacultad(req: Request, res: Response) {
  const { id } = req.params;
